...
raw = open("/home/genic/Documents/Coding/microgpt/input.txt", encoding="utf-8").read()
...
# Tokenizer: tags are atomic, remaining text is character-level
raw_vocab_text = ''.join(docs)

def tokenize_text(s):
    # Keep tags like <HAIKU>, <L1>, <5>, <END> as single tokens.
    # Everything else becomes individual characters.
    # out = []
    # i = 0
    # while i < len(s):
    #     if s[i] == '<':
...
itos = sorted(set(tok for doc in tokenized_docs for tok in doc))
BOS = len(itos)
itos.append("<BOS>")
...
for step in range(num_steps):
    ...
    tokens = [BOS] + [stoi[tok] for tok in tokenize_text(doc)] + [BOS]
...
temperature = 0.25
print("\n--- inference (new, hallucinated haikus) ---")
